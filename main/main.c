...
static void publish_luminosity(MQTT_CLIENT_DATA_T *state) {
    ...
    adc_select_input(1);
    const char *light_sensor_key = full_topic(state, "/light_sensor");

    uint16_t ldr_value = adc_read();

    char ldr_str[16];
    snprintf(ldr_str, sizeof(ldr_str), "%d", ldr_value);
    INFO_printf("Publishing %s to %s\n", ldr_str, light_sensor_key);
    mqtt_publish(state->mqtt_client_inst, light_sensor_key, ldr_str, strlen(ldr_str), MQTT_PUBLISH_QOS, MQTT_PUBLISH_RETAIN, pub_request_cb, state);
}
...
static void sub_unsub_topics(MQTT_CLIENT_DATA_T *state, bool sub) {
    ...
    mqtt_sub_unsub(state->mqtt_client_inst, full_topic(state, "/light"), MQTT_SUBSCRIBE_QOS, cb, state, sub);
}
...
static void mqtt_incoming_data_cb(void *arg, const u8_t *data, u16_t len, u8_t flags) {
    ...
    } else if (strcmp(basic_topic, "/light") == 0) {
        gpio_init(LED_PIN);
        gpio_set_dir(LED_PIN, GPIO_OUT);

        uint16_t ldr_value = atoi((const char *)state->data);
        if (ldr_value < 1400) {
            gpio_put(LED_PIN, 1);
        } else {
            gpio_put(LED_PIN, 0);
